...
def _seed_equity(s: Store) -> None:
    s.record_equity("default", "2024-01-01T00:00:00+00:00", 10000.0, 10000.0)
    s.record_equity("default", "2024-01-01T01:00:00+00:00", 10100.0, 9000.0)
    s.record_equity("default", "2024-01-01T02:00:00+00:00", 10250.0, 9000.0)


def test_equity_series_oldest_to_newest_limited():
    s = Store(":memory:")
    _seed_equity(s)
    series = s.equity_series("default", limit=2)
...
def test_equity_series_all_when_limit_large():
    s = Store(":memory:")
    _seed_equity(s)
    series = s.equity_series("default", limit=100)
...
def test_recent_fills_newest_first():
    ...
    s.record_fill("default", "t2", Fill("BTC/USDT", Side.SELL, 0.5, 110.0, 0.055))
    fills = s.recent_fills("default", limit=10)
    assert len(fills) == 2
...
def test_recent_decisions_includes_indicators():
    s = Store(":memory:")
    s.record_decision("default", "t1", "BTC/USDT", "BUY", "cruce alcista", 30.5, 29.0, 41.0)
